...
def test_contract_constants():
    ...
    assert CONTRACT_END_DATE == "2027-03-31"
...
def test_validate_contract_date():
    ...
    # Test out of bounds (before June 2026 or after March 2027)
    try:
        validate_contract_date("2026-05-31")
        assert False, "Should have raised HTTPException for pre-contract date"
    except HTTPException as e:
        assert e.status_code == 400

    try:
        validate_contract_date("2027-04-01")
...
def test_parse_generation_csv_2027():
    sample_csv = (
        ...
        "2027-03-15,1,8.5,45.2\n"
        "2027-03-15,2,9.0,50.0\n"
        "2027-03-31,96,7.2,12.5\n"
    )
    rows = parse_generation_csv(sample_csv)
    assert len(rows) == 3
    assert rows[0]["date"] == "2027-03-15"
    assert rows[0]["block"] == 1
    assert rows[2]["date"] == "2027-03-31"
