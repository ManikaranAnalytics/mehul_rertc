...
import { CONTRACT_DATES, formatContractDateLabel } from '../../utils/constants';
...
export default function CarryForwardBar() {
  ...
  return (
    <div style={{
      ...
      {/* Next Day button */}
      {CONTRACT_DATES.indexOf(selectedDate) < CONTRACT_DATES.length - 1 && (
        <button
        ...
        >
          Roll to {formatContractDateLabel(CONTRACT_DATES[CONTRACT_DATES.indexOf(selectedDate) + 1] ?? '')} →
        </button>
      )}
