...
Open http://localhost:5173 — Vite proxies `/api` to http://localhost:9000.

If `npm` is not on your PATH, use the portable Node under the repo `.tools/` directory or install Node 20+.

**Requires a running backend** on port 9000 (see [backend/README.md](../backend/README.md)).
...
|--------------------|-------------|
| `http://host.docker.internal:9000` | Backend on host (uvicorn or backend container publishing :9000) — **Mac/Windows Docker** |
| `http://172.17.0.1:9000` | Backend on host — **Linux Docker** (or use host IP) |
| `http://backend:9000` | Backend on same Docker Compose network (root full-stack compose) |
